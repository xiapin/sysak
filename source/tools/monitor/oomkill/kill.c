...
static void notify_process_killed(const poll_loop_args_t* args, const procinfo_t* victim)
{
    ...
    if (args->notify) {
        ...
        notify_dbus("oomkill", notif_args);
    }
    if (args->notify_ext) {
...
procinfo_t find_largest_process(const poll_loop_args_t* args)
{
    ...
    if (victim.pid == getpid()) {
        warn("%s: selected myself (pid %d). Do you use hidpid? See https://github.com/rfjakob/oomkill/wiki/proc-hidepid\n",
            __func__, victim.pid);
        // zero victim struct
...
void kill_process(const poll_loop_args_t* args, int sig, const procinfo_t* victim)
{
    if (victim->pid <= 0) {
        ...
        if (args->notify) {
            notify_dbus("oomkill", "Error: Could not find a process to kill. Sleeping 1 second.");
        }
        sleep(1);
        return;
    }
    ...
    if (res != 0) {
        ...
        if (args->notify) {
            notify_dbus("oomkill", "Error: Failed to kill process");
        }
