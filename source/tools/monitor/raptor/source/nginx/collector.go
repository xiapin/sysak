...
import (
	...
	"github.com/hpcloud/tail" //用于tail文件
	"github.com/prometheus/client_golang/prometheus"
	"github.com/satyrius/gonx" //用于nginx日志内容解析
)
...
func NewCollector(cfg *AppConfig) *Collector {
	...
	return &Collector{
		...
		cfg:    cfg,
		parser: gonx.NewParser(cfg.Format), // 配置解析格式
	}
}
		...
		go func() {
			for line := range t.Lines {
				...
				if err != nil {
					...
					//fmt.Printf("error while parsing line %s", err)
					continue
				}
