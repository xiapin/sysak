...
void get_path_btf(char *buf, int buf_size,  char *path, int path_size)
{
	FILE *fp = NULL;
	char version[64] = {};
	char *sysak_env_path = NULL;
	char btf_path_buf[BTF_PATH_MAX] = {0};
	...
	if (path) {
		snprintf(btf_path_buf, BTF_PATH_MAX, "%s", path);
	}
	if (access(btf_path_buf, F_OK) != 0) {
		if (access("/sys/kernel/btf/vmlinux", F_OK) == 0) {
			snprintf(btf_path_buf, BTF_PATH_MAX,
...
#endif
	//fprintf(stdout, "==========%s\n", btf_path_buf);
	snprintf(buf, BTF_PATH_MAX, "vmlinux-%s", btf_path_buf);
	pclose(fp);
}