...
// assume CONFIG_SPARSEMEM_VMEMMAP is set
# define PFN_TO_PAGE(pfn)    (vmemmap + (pfn) * PAGE_STRUCT_SIZE)
# define is_kvaddr(kvaddr) (!!(kvaddr >= PAGE_OFFSET))
# define SECTION_SIZE_BITS	30   // arch/arm64/include/asm/memory.h
#else /*x86 arch*/
...
/*
 * Marco used to acheive __pfn_to_sections.
 * we assume CONFIG_SPARSEMEM and CONFIG_SPARSEMEM_VMEMMAP is set,
 * so NODE_NOT_IN_PAGE_FLAGS is define, use __pfn_to_section defined in mmzone.h
 */
extern unsigned long mem_section_base;
extern int mem_section_size;

#define PFN_SECTION_SHIFT           (SECTION_SIZE_BITS - page_shift)
#define pfn_to_section_nr(pfn)      ((pfn) >> PFN_SECTION_SHIFT)
/*
 * We assume CONFIG_SPARSEMEM_EXTREME is set,
 */
#define SECTIONS_PER_ROOT           (PAGE_SIZE / mem_section_size)
