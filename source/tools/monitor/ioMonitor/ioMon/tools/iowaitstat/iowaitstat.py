...
global_iowaitstat_stop = False


def signal_exit_handler(signum, frame):
    global global_iowaitstat_stop
    global_iowaitstat_stop = True
...
class iowaitClass():
    def __init__(self, pid, cycle, top, json, iowait_thresh):
        ...
        for kprobe,retProbe in {'io_schedule_timeout':True, 'io_schedule':True}.items():
            if supportKprobe(kprobe) == False:
                ...
                continue
            self.expression.append('p:p_%s_0 %s' % (kprobe, kprobe))
            self.kprobe.append('p_%s_0' % kprobe)
            if retProbe == True:
                self.expression.append('r:r_%s_0 %s' % (kprobe, kprobe))
    ...
    def showJson(self, stat, totalTimeout, gloabIowait):
        ...
        if len(iowaitStatDicts["iowait"]) > 0:
            self.writeDataToJson(json.dumps(iowaitStatDicts))
...
def iowaitstatStart(argv):
  ...
  ./iowaitstat.py -p [PID] -c 1
    ...
    if argv is None:
        ...
        if args.Timeout is not None:
            ...
            if argv is None:
                ...
                timer = threading.Timer(int(timeoutSec), exit_handler)
                timer.start()
    ...
    while global_iowaitstat_stop != True:
        c.entry(interval)
        if loop == False:
            break
    c.clear()
