...
def get_description():
    return '[IO]write_classid softlockup'
...
def get_issue_keywords():
    return ['softlockup','4.9','write_classid']
    ...
    try:
        ...
        if len(softlockup_cts) > 0:
            for ct in softlockup_cts:
                if "write_classid" in ct["softlockupkey"]:
                    ret['return'] = True
                    ret['solution'] = utils.format_result(desc=("write_classid() softlockup"),
                        commitid=("https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=a90e90b7d55e789c71d85b946ffb5c1ab2f137ca"))
