...
void *shm_handler(void *arg)
{
	...
	for (i = 3; i >= 0; i--) {
		...
		printf("**delay=%lld stamp=%lld cpu=%d pid=%d task=%s\n",
...
int main(int argc, char *argv[])
{
	...
	begin = 0;
	fd = shm_open("rqslwselftest", O_CREAT|O_RDWR|O_TRUNC, 06666);
	...
	fp = popen("sysak runqslower -S rqslwselftest -s 20 2", "r");
	if (!fp) {
		perror("popen runqslower");
		return -1;
	}
	...
	ret = pclose(fp);
	if (ret == -1) {
		perror("pclose runqslower");
		return -1;
	}
	end = 1;
cleanup:
	munmap(p, sizeof(struct summary) + 32);
	ret = shm_unlink("rqslwselftest");
	if (ret < 0) {
		ret = errno;
		perror("shm_unlink");
		return ret;
