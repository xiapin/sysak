...
impl<'a, 'b> PerfBufferBuilder<'a, 'b> {
    ...
    pub fn build(self) -> Result<PerfBuffer<'b>> {
        if self.map.map_type() != MapType::PerfEventArray {
            bail!("Must use a PerfEventArray map".to_string(),)
        }

        if !is_power_of_two(self.pages) {
            bail!("Page count must be power of two".to_string(),)
        }
        ...
        if err != 0 {
           bail!("Unable to create perf buffer: {}", err)
        } else {
            Ok(PerfBuffer {
                ptr,
...
impl<'b> PerfBuffer<'b> {
    ...
    pub fn poll(&self, timeout: Duration) -> Result<()> {
        ...
        if ret < 0 {
            bail!("Err({}) occurs on perf poll", ret)
        } else {
            Ok(())
        }
    }
