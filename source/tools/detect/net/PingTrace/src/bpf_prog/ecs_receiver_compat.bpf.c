...
int icmp_rcv_probe(struct pt_regs *ctx)
{
        ...
        tag_timestamp_compat(&id_map, &pt_map, skb, P_R_RX_ICMPRCV, ICMP_ECHO);
        return 0;
}
...
int net_dev_start_xmit_hook(struct net_dev_xmit_args *args)
{
        ...
        tag_timestamp_compat(&id_map, &pt_map, skb, P_R_TX_DEVOUT, ICMP_ECHOREPLY);
        return 0;
}
...
int netif_rx_hook(struct netif_receive_skb_args *args)
{
        ...
        tag_timestamp_compat(&id_map, &pt_map, skb, P_R_RX_IPRCV, ICMP_ECHO);
        return 0;
}
