...
from diskstatClass import getDevtRegion
    ...
    def show(self):
        ...
        for key, item in stat:
            ...
            if self.Pattern:
                ...
                patPercent = ('%-12s%-12s%-12s%-12s%-12s%-12s%-12s%s' % (
            ...
            item["bps_wr"] = \
                humConvert(item["bps_wr"], True).replace('s', stSecs) if item["bps_wr"] else 0
            print('%-20s%-8s%-12s%-16s%-12s%-12s%-12s%s' % (item["comm"],
                str(item["pid"]), str(item["iops_rd"]), item["bps_rd"], 
                str(item["iops_wr"]), item["bps_wr"], item["device"],
                patPercent))
        if self.Pattern:
